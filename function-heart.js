...
function drawTextCenter() {
  ...
  // ทำให้ข้อความขยับขึ้นลงนิด ๆ ด้วย wave
  let waveOffset = Math.sin(performance.now() / 300) * 10;

  a.save();
  a.font = 'bold 24px Arial';
  a.textAlign = 'center';

  // ไล่ระดับสี: ม่วง → ชมพู → แดงออกม่วง
  let gradient = a.createLinearGradient(centerX - 50, centerY, centerX + 50, centerY);
  gradient.addColorStop(0, '#8e24aa');
  gradient.addColorStop(0.5, '#ec407a');
  gradient.addColorStop(1, '#c2185b');

  a.fillStyle = gradient;
  a.fillText('รักนะไอ้ซาลาเปาของเค้า', centerX, centerY + waveOffset);
  a.restore();
}
